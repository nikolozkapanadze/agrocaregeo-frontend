...
export default function AIAgentPage(): React.ReactElement {
  ...
  const runAnalysis = async (question?: string) => {
    ...
    try {
      ...
      setMessages(prev => {
        const withoutLoading = prev.filter(m => !m.loading)
        return [...withoutLoading, { role: 'assistant', content: `❌ ${errorMsg}` }]
      })
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="relative h-[calc(100vh-4rem)] -m-5 lg:-m-6 flex flex-col">
      {/* Header */}
      <div className="shrink-0 border-b border-white/10 bg-bg-card/50 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-text-primary">AI აგრონომ-ასისტენტი</h1>
            <p className="text-xs text-text-muted">
              რეალური ფერმერული მონაცემების ანალიზი და რეკომენდაციები
            </p>
          </div>
          <div className="ml-auto flex items-center gap-3">
            <button
              onClick={() => setShowSettings(true)}
              className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-text-secondary hover:bg-white/5 hover:text-text-primary border border-white/10 transition-colors"
            >
      ...
      {/* Settings overlay */}
      {showSettings && (
        <div className="absolute inset-0 z-20 bg-bg-card overflow-y-auto p-6">
          ...
          <div className="flex flex-col items-center justify-center h-full text-center space-y-4 text-text-muted">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs max-w-[300px]">
                მონაცემები იტვირთება... AI ანალიზი მოიცავს სატელიტურ, ამინდის, მინერალურ და ფენოლოგიურ მონაცემებს.
              </p>
            </div>
          </div>
        )}
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="shrink-0 border-t border-white/10 bg-bg-card/80 backdrop-blur-xl px-6 py-4">
        <div className="flex items-center gap-2">
          <input
            ref={inputRef}
            type="text"
            ...
            placeholder="დასვით კითხვა თქვენი ფერმერული მონაცემების შესახებ..."
            disabled={isLoading}
            className="flex-1 rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent/40 focus:ring-1 focus:ring-accent/20 transition-all disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={isLoading || !input.trim()}
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent text-white hover:bg-accent-hover disabled:opacity-40 disabled:cursor-not-allowed transition-all"
          >
